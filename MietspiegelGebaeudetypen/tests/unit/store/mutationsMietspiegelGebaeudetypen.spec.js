...
describe("addons/MietspiegelGebaeudetypen/store/mutationsMietSpiegelGebaeudetypen", function () {
    describe("applyTranslationKey", function () {
        it("removes 'translate#' from name if present", function () {
            const state = {
                ...
                payload = "translate#additional:modules.tools.mietspiegel.gebaeudetypen.title";

            applyTranslationKey(state, payload);
            expect(state.name).to.equals("additional:modules.tools.mietspiegel.gebaeudetypen.title");
        });
