...
const PendlerCoreModel = Tool.extend(/** @lends PendlerCoreModel.prototype */{
    defaults: _.extend({}, Tool.prototype.defaults, {
        ...
        dataType: "WFS",
        attrKreis: "kreisname",
        kreiseToIgnore: ["Bremen", "Berlin", "Kiel", "Hannover"]
    }),
    /**
    ...
    initialize: function () {
        ...
        else if (this.get("dataType") === "GeoJSON") {
            const features = this.requestFeatures(),
                kreise = this.parseKreiseFromFeatures(features, this.get("attrKreis"), this.get("kreiseToIgnore"));

            this.setKreise(kreise);
            this.setFeatures(features);

            this.listenTo(this, {
                "change:kreis": function (model, value) {
                    const gemeinden = this.parseGemeindenFromFeatures(model.get("features"), model.get("attrGemeinde"), model.get("attrKreis"), value);

                    this.unset("gemeinde");
                    this.unset("direction", {silent: true});
                    this.clear();
                    this.setGemeinden(gemeinden);
                },
                "change:gemeinde": function () {
                    this.unset("trefferAnzahl", {silent: true});
                    this.unset("direction", {silent: true});
    ...
    requestFeatures: function () {
        const format = new GeoJSON(),
            ...
            proxyUrl = Radio.request("Util", "getProxyURL", this.get("url"));
        let features = [];
    ...
    parseKreiseFromFeatures: function (features, attrKreis, kreiseToIgnore) {
        let kreise = this.getAllValuesByAttribute(features, attrKreis);

        kreise = kreise.filter(kreis => {
            return !kreiseToIgnore.includes(kreis);
        });
        return kreise;
    },

    parseGemeindenFromFeatures: function (features, attrGemeinde, attrKreis, kreis) {
        let gemeindenByKreis = [];

        if (Array.isArray(attrKreis)) {
            attrKreis.forEach(attrKreisPart => {
                const gemeindenByKreisPart = features.filter(feature => {
                    return feature.get(attrKreisPart) === kreis;
                });

                gemeindenByKreisPart.forEach(gemeindeByKreisPart => {
                    gemeindenByKreis.push(gemeindeByKreisPart);
                });
            });
        }
        else {
            gemeindenByKreis = features.filter(feature => {
                return feature.get(attrKreis) === kreis;
            });
        }

        this.setLineFeatures(gemeindenByKreis);
        gemeindenByKreis = this.getAllValuesByAttribute(gemeindenByKreis, attrGemeinde);
        return gemeindenByKreis;
    },

    getAllValuesByAttribute: function (features, attr) {
        let values = [];

        if (Array.isArray(attr)) {
            attr.forEach(attrPart => {
                features.forEach(feature => {
                    values.push(feature.get(attrPart));
                });
            });
        }
        else {
    ...
    parseKreise: function (data) {
        const kreise = [],
            ...
            attrKreis = this.get("attrKreis");
        let kreis;

        _.each(hits, function (hit) {
            kreis = $(hit).find("app\\:" + attrKreis + "," + attrKreis)[0].textContent;
            kreise.push(kreis);
        });
